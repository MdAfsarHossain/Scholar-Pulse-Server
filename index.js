...
async function run() {
  try {
    ...
    // Delete Specific Scholarships data
    app.delete(
    ...
    // Delete applied application
    app.delete("/delete-application/:id", async (req, res) => {
      const id = req?.params.id;
      const query = { _id: new ObjectId(id) };
      const result = await allApplicationsCollection.deleteOne(query);
      res.send(result);
    });
    ...
    app.get("/scholarships-count", async (req, res) => {
      const filterData = req?.query?.filter;
      const search = req?.query?.search;
      ...
      const count = await allScholarShipsCollection.countDocuments(query);
